...
from fury.utils import get_spherical_uvs
...
verts, faces = prim_sphere(phi=32, theta=16)
texcoords = get_spherical_uvs(verts)
earth_actor = surface(verts, faces, texture=earth_filename, texture_coords=texcoords)
...
moon_actor = surface(verts, faces, texture=moon_filename, texture_coords=texcoords)
...
moon_actor.local.position = (2, 0.1, 0.5)
moon_actor.local.scale = 0.27
earth_actor.local.rotation = la.quat_from_axis_angle([1, 0, 0], -1.57)
moon_actor.local.rotation = la.quat_from_axis_angle([1, 0, 0], -1.57)
...
showm = ShowManager(scene=scene, size=(900, 768), title="Earth Animation")
